...
import { Wallet, LogOut, Zap, AlertCircle, CheckCircle2 } from 'lucide-react';
  ...
  if (state.status === 'ready') {
    ...
    return (
      <div className="flex items-center gap-2 bg-black/60 backdrop-blur-md border border-[#90EE90]/50 rounded-full px-3.5 py-1.5 shadow-lg">
        ...
        <span className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-[#90EE90]/20 text-[#90EE90] font-semibold tracking-wide border border-[#90EE90]/30">
          <CheckCircle2 className="w-3 h-3" />
          <span>{state.isDemo ? 'Testnet Wallet' : 'zkLogin Active'}</span>
        </span>
        <button
          onClick={logout}
          className="text-[11px] text-white/60 hover:text-white transition-colors ml-1 flex items-center gap-1 hover:underline cursor-pointer"
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="flex items-center gap-2">
        <button
          onClick={login}
          className="group flex items-center gap-2.5 bg-white text-black font-semibold px-4 py-2 rounded-full shadow-2xl hover:scale-105 active:scale-95 transition-all text-xs md:text-sm cursor-pointer"
        >
        ...
        <button
          onClick={loginDemo}
          className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 text-white/90 border border-white/20 font-medium px-3.5 py-2 rounded-full backdrop-blur-md shadow-lg hover:scale-105 active:scale-95 transition-all text-xs cursor-pointer"
        >
